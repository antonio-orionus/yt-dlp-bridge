import type {
  ...
  ParsedJsonLines,
  ProgressEvent,
  SanitizedFormatSummary,
  SanitizedMetadataItem,
  SubtitleSummary,
  ThumbnailSummary
} from "./types.js";
...
export function parseFormats(output: string): FormatSummary[] {
  ...
  return lines.map((raw) => {
    ...
    const format: FormatSummary = {
      formatId: columns[0] ?? raw.trim(),
      raw
    };
...
export function parseSubtitles(output: string): SubtitleSummary[] {
  ...
  for (const raw of output.split(/\r?\n/)) {
    ...
    const alignedColumns = /^[A-Za-z0-9][A-Za-z0-9_-]*(?:-[A-Za-z0-9]+)*\s{2,}/.test(line)
      ...
      : [];
    const fallback = line.match(/^([A-Za-z0-9][A-Za-z0-9_-]*(?:-[A-Za-z0-9]+)*)\s+(.+?)\s+((?:vtt|srt|ttml|srv3|srv2|srv1|json3)(?:,\s*(?:vtt|srt|ttml|srv3|srv2|srv1|json3))*)$/i);
    const columns = alignedColumns.length >= 2 ? alignedColumns : fallback ? [fallback[1] ?? "", fallback[2] ?? "", fallback[3] ?? ""] : [];
    if (columns.length < 2) continue;

    const subtitle: SubtitleSummary = {
      language: columns[0] ?? line,
      formats: splitFormats(columns.at(-1) ?? ""),
      source,
      raw
    };
...
export function parseThumbnails(output: string): ThumbnailSummary[] {
  return dataLines(output).map((raw) => {
    const columns = raw.trim().split(/\s+/);
    const item: ThumbnailSummary = {
      id: columns[0] ?? raw.trim(),
      raw
    };
...
export function parseProgress(output: string): ProgressEvent[] {
  ...
  for (const raw of output.split(/\r?\n/)) {
    ...
    const total = line.match(/\bof\s+([^\s]+(?:\s+[^\s]+)?)/i);
    const speed = line.match(/\bat\s+([^\s]+)/i);
    const eta = line.match(/\bETA\s+([^\s]+)/i);
    const event: ProgressEvent = {
      phase: isDownload ? "download" : isPostprocess ? "postprocess" : "unknown",
      raw
    };
...
export function parseFinalPaths(stdout: string): FinalPathResult {
  ...
  for (const line of stdoutLines) {
    ...
    const subtitle = line.match(/^\[info\]\s+Writing video subtitles to:\s+(.+)$/i);
    if (subtitle?.[1]) paths.add(subtitle[1].trim());
    const thumbnail = line.match(/^\[info\]\s+Writing video thumbnail(?:\s+\d+)?\s+to:\s+(.+)$/i);
    if (thumbnail?.[1]) paths.add(thumbnail[1].trim());
    const converted = line.match(/^\[(?:SubtitlesConvertor|ThumbnailsConvertor)\]\s+Converting .+? to (.+)$/i);
