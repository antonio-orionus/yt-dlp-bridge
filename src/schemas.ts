...
export const SponsorBlockOptionsSchema = z.object({
...
export const PlanDownloadInputSchema = z.object({
  url: UrlSchema,
  kind: z.enum(["video", "audio", "subtitles", "thumbnail", "playlist"]).default("video"),
...
export const DownloadInputSchema = PlanDownloadInputSchema.extend({
...
export const ValidateOptionsInputSchema = PlanDownloadInputSchema.partial().extend({ url: UrlSchema.optional() });
export const ArchivePathInputSchema = z.object({ archivePath: z.string(), entry: z.string().optional() });
export const ExpertInputSchema = z.object({ url: UrlSchema.optional(), args: z.array(z.string()).default([]), dryRun: z.boolean().default(true), output: OutputPolicySchema.default({ allowOverwrite: false }) });
