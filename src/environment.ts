...
async function detectBinary(name: string, command: string, args: string[], requiredFor: string[], required: boolean, config: ServerConfig): Promise<DetectedDependency> {
  try {
    const result = await runCommand(command, args, {
      timeoutMs: Math.min(config.defaultTimeoutMs, 10_000),
    ...
    return {
      ...
      requiredFor,
      notes: required ? [] : ["Optional dependency; required only for matching workflows."]
    };
  } catch (error) {
    return {
      name,
      ...
      command,
      requiredFor,
      notes: [required ? "Required dependency is not available." : "Optional dependency is not available.", error instanceof Error ? error.message : String(error)]
    };
  }
}

function cookieStatus(config: ServerConfig): EnvironmentReport["cookies"] {
  const status: EnvironmentReport["cookies"] = {
    fileConfigured: Boolean(config.cookiesFile),
    browserConfigured: Boolean(config.cookiesFromBrowser)
  };
