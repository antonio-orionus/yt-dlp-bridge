import { classifyYtDlpStderr, errorKindMetadata, extractLastError, type YtDlpErrorKind } from "ytdlp-errors";
...
export function toStructuredError(error: unknown): StructuredError {
  if (error instanceof CommandExecutionError) {
    const stderr = error.result.stderr || error.result.stdout || error.message;
    ...
    const structured: StructuredError = {
      kind,
      message: extractLastError(stderr) || error.message,
      retryable: metadata.recoverable,
      exitCode: error.result.exitCode,
      stderrExcerpt: excerpt(stderr),
      suggestedFixes: suggestedFixes(kind, metadata.suggestedFlags)
    };
  ...
  if (error instanceof Error) {
    return {
      ...
      retryable: false,
      stderrExcerpt: excerpt(error.stack ?? error.message),
      suggestedFixes: []
    };
  }
...
function suggestedFixes(kind: YtDlpErrorKind | string, flags: readonly string[] | undefined): string[] {
